#!/usr/bin/env python3
"""
Test script for Hyperliquid adapter
...
from adapters.hyperliquid import HyperliquidAdapter, HyperliquidConfig
from common.types import Side
from config import CFG


async def test_hl():
    """Test Hyperliquid adapter connection and basic functionality"""
    logger.info("=== Testing Hyperliquid Adapter ===")

    # Create adapter configuration
    config = HyperliquidConfig(
        private_key=CFG.hyperliquid_private_key,
        account_address=CFG.hyperliquid_account_address if CFG.hyperliquid_account_address else None,
        pair=CFG.pair,
        env=CFG.hyperliquid_env
    )
    ...
    try:
        ...
        logger.info(f"✅ Connected to Hyperliquid ({config.env})")
        logger.info(f"Trading pair: {config.pair}")
        logger.info(f"Account: {adapter.address}")
        ...
        logger.info("\n✅ Hyperliquid adapter test completed successfully")
        return True

    except Exception as e:
        logger.error(f"❌ Hyperliquid test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
async def test_position(adapter: HyperliquidAdapter):
    """Test position retrieval and display"""
    try:
        coin = adapter._format_coin(adapter.cfg.pair)
        position = await adapter.get_position(coin)

        logger.info(f"Coin: {position['symbol']}")
...
async def test_bbo_stream(adapter: HyperliquidAdapter, max_count: int = 10) -> int:
    """Test BBO streaming"""
    count = 0
...
async def main():
    ...
    if not CFG.hyperliquid_private_key:
        logger.error("HYPERLIQUID_PRIVATE_KEY not configured in .env")
        logger.info("Please add HYPERLIQUID_PRIVATE_KEY=0x... to your .env file")
        return
