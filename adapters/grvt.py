...
class GrvtAdapter:
    ...
    async def _subscribe_websocket_streams(self):
        ...
        try:
            ...
            # 1. Subscribe to BBO stream (mini.s - lighter than ticker.s)
            ...
            loguru_logger.info(f"[GRVT] Subscribed to BBO stream for {instrument}")

            # 2. Subscribe to fill stream (private fills)
            if self.cfg.private_key:
